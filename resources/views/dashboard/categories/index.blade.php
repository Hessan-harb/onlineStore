@extends('layouts.dashboard')

@section('content')


    <div class="main-wrapper">

        <div class="page-wrapper">
            <div class="page-content">

                <div class="d-flex justify-content-between align-items-center flex-wrap grid-margin">

            
                    <div class="container justify-content-center ">
                        <h2 style="font-size: 30px;" class="text-right-15 ">All Categories</h2>
                    </div>

                    <div class="container pt-6 justify-content-center">

                         <div class="mb-5">
                            @if(Auth::user()->can('categories.create'))
                             <a href="{{route('categories.create')}}" class="btn btn-primary m-9 ">Add Category</a>
                             @endif
                        </div>

                        <div class="mb-5">
                             <a href="{{route('categories.trash')}}" class="btn btn-primary m-9 ">Trash Category</a>
                        </div>

                        <x-alert type="success" />

                        <form action="{{URL::current()}}" method="get" class="pb-3">
                            <x-form.input name="name" />
                            
                            <button class="btn btn-danger">Filter</button>
                        </form>

                        <table class="table table-bordered">
                            <thead>
                                <tr>
                                <th scope="col">id</th>
                                <th scope="col">Name</th>
                                <th scope="col">parent</th>
                                <th scope="col">Products</th>
                                <th scope="col">created_at</th>
                                <th scope="col"></th>
                                </tr>
                            </thead>
                            <tbody>
                                @if($categories->count())
                                @foreach($categories as $category)
                                <tr>
                                <th scope="row">{{$category->id}}</th>
                                <td> <a href="{{route('categories.show',$category->id)}}">{{$category->name}}</a> </td>
                                <td>{{$category->parent->name ?? ''}}</td>
                                <th>{{$category->products_number}}</th>
                                <td>{{$category->created_at}}</td>
                                <td><img src="{{asset('storage/' . $category->image)}}" alt=""></td>
                                <td>
                                    @can('categories.update')
                                    <a href="{{route('categories.edit',$category->id)}}" class="btn btn-success">Edit</a>
                                    @endcan
                                </td>
                                <td>
                                    @can('categories.delete')
                                    <form action="{{route('categories.destroy',$category->id)}}" method="post">
                                        @csrf
                                        @method('delete')
                                        <button class="btn btn-danger" type="submit">Delete</button>
                                    </form>
                                    @endcan
                                </td>
                                </tr>
                                @endforeach
                                @else
                                <tr>
                                    <td colspan="7"> No Categories Find</td>
                                </tr>
                                @endif
                                
                            </tbody>
                        </table>
                    </div>
                    

                </div>
                {{$categories->withQueryString()->links()}}
            </div>
        </div>
    </div>


@endsection